...
import { LOCAL_ADVICE, LOCAL_SUCCESS_POINTS } from './local-content';
...
export function getLocalAdvice(jobTypeId: string, prefId: string): string {
  ...
  // 差別化済みの県は固有コンテンツを優先（lib/local-content.ts）
  const localAdvice = LOCAL_ADVICE[prefId]?.[jobTypeId];
  if (localAdvice) return localAdvice;
...
export function getSuccessPoints(jobTypeId: string, prefId: string): SuccessPoint[] {
  ...
  // 差別化済みの県は固有の成功ポイントを優先（lib/local-content.ts）
  const localPoints = LOCAL_SUCCESS_POINTS[prefId]?.[jobTypeId];
  if (localPoints) return localPoints;
