...
export function getLocalAdvice(jobTypeId: string, prefId: string): string {
  ...
  if (prefId === 'tokyo') {
    const tokyoAdvice: Record<string, string> = {
      ...
      landscaping: '東京都が推進する「2030年東京グリーンビズ」計画と民間再開発の緑地整備義務化で都市造園の求人は拡大傾向です。麻布台ヒルズ庭園緑地・神宮外苑緑地計画・東京港臨海部広域防災公園の整備など大規模民間・公共造園案件が並行して進んでいます。1級造園施工管理技士に加え屋上緑化・植栽基盤設計の経験があると年収500万円台への引き上げが現実的なエリアです。',
    };
...
export function getSuccessPoints(jobTypeId: string, prefId: string): SuccessPoint[] {
  ...
  if (prefId === 'tokyo') {
    const tokyoPoints: Record<string, SuccessPoint[]> = {
      ...
      landscaping: [
        { title: '都市緑化・屋上緑化の施工経験を武器にする', body: '東京グリーンビズ推進と民間再開発の緑地整備義務で屋上・壁面緑化の施工需要が急増しています。一般造園と異なる技術が必要で専門性が高く評価され、1級造園施工管理技士と組み合わせると希少人材として扱われます。' },
        { title: '大手ゼネコン・デベロッパー系再開発の造園実績を積極的にアピールする', body: '麻布台・神宮外苑・品川など大型民間再開発の造園工事は大手ゼネコンの協力会社として受注するケースが大半です。このような大規模案件の施工実績は転職市場で特に高く評価されます。' },
        { title: '植栽基盤設計・樹木医資格で年収500万円台への引き上げを狙う', body: '都市部の造園は植栽基盤設計や既存樹木の保護計画が重要視されており、樹木医資格や植栽基盤設計の経験があると年収500万円台を超える求人を狙えます。公共造園工事の施工実績と組み合わせてアピールしましょう。' },
      ],
    };
